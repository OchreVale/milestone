...
QP_TMDB = {"api_key": os.getenv("TMDB_KEY")}
...
def home():
    ...
    if flask.request.method == "POST":
        ...
        for i, result in enumerate(results):
            ...
            youtube.append(
                functions.query_pararms(name=titles[i], id_=str(result["id"]))
            )
...
def delete():
    ...
    movie = flask.request.args.get("id")
    cid_ = data
    query = Reviews.query.filter_by(user_id=cid_, movie_id=movie).first()
    if (isinstance(query, NoneType)) is False:
...
@bp.route("/review and comments")
def index():
    """
    Hooks the react server to the flask server.
    """
